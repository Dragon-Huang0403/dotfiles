...
# VIM-PLUG
# ============================================================================
# ~/.vimrc is generated by BOOKSTRAP (not stowed here) and calls
# plug#begin/plug#end. Without plug.vim vim errors out on every startup,
# including when git opens it as the commit editor.

dotfiles_echo "Setting up vim-plug..."

VIM_PLUG="${HOME}/.vim/autoload/plug.vim"
if [ ! -f "$VIM_PLUG" ]; then
  dotfiles_echo "Installing vim-plug..."
  curl -fsSL --create-dirs -o "$VIM_PLUG" \
    https://raw.githubusercontent.com/junegunn/vim-plug/master/plug.vim
else
  dotfiles_echo "vim-plug already installed. Skipping."
fi

# Install the plugins declared in ~/.vimrc. Failures here (offline, proxy)
# must not abort setup, hence the `|| true` against `set -e`.
if [ -f "${HOME}/.vimrc" ] && command -v vim >/dev/null; then
  dotfiles_echo "Installing vim plugins..."
